...
Static.dom.style = function (element, properties, value) {
	if ( typeof properties == "object" ) {
		for (prop in properties) {
			if (properties.hasOwnProperty(prop)) {
				...
				if ( element instanceof HTMLElement ) {
					// we can use HTMLElement.style
					element.style[prop] = properties[prop];
				} else if ( element instanceof Element ) {
					// we can't
					var styles = (element.getAttribute("style") || "").split(";");
					var replacedStyle = false;
					Static.map(styles, function (style, i) {

						if ( prop == Static.matches(style, /(\S+)\:/)[1] ) {
					...
					if ( !replacedStyle ) {
						styles.push( prop + ":" + properties[prop] );
					}
		...
		if ( element instanceof HTMLElement ) {
			...
			element.setAttribute("style", properties + ":" + value + ";" + (element.getAttribute("style") || "") );
		}
	}
}
...
function AjaxNav( options ) {
	...
	function initialize() {
		...
		if ( !(options.selector && Static.isArrayLike(options.items)) ) {
			return;
		}
	...
	function onItemClick(event) {
		...
		var anchor = Static.dom.closest(event.target, "a");
		if ( anchor && anchor.href ) {

			if ( !Static.http.isSamePath(anchor.href, document.location.href) ) {

				load( anchor.href, true );
			}
		}
	}
...
console.log("load");
		...
		Static.http.requestDom(href, function (dom) {
			promise.all(fadeingPromises).then(function () {
				onDocumentLoad( dom, href, pushState );
			});
		}, onDocumentLoadError, {});
	}


	function onDocumentLoad( dom, href, pushState ) {
		...
		var domCurrent = document.querySelector(options.selector);
		if ( !domCurrent ) {

			return console.error("cannot find current body");
		}

		var domNew = dom.querySelector(options.selector);
		var parent = domCurrent.parentNode;
console.log("domNew", domNew, domNew.style);
		if ( !domNew || !parent ) {
			document.location.href = href;
			return;
		}

		// domNew.setAttribute("style", "display: none");
		domNew.style.display = "none";

		onBeforeChange( dom, href );

		parent.insertBefore(domNew, domCurrent);
		parent.removeChild(domCurrent);

		// domNew.setAttribute("style", "display: ''");

		domNew.style.display = "";

		onAfterChange( dom, href );
	...
	function onAfterChange( href ) {

		fadeIn();

		if ( typeof options.onAfterChange == "function" ) {
			options.onAfterChange( href );
		}
	}
...
element.style.transform = "scale(1)";
						console.log(element, propsFrom, propsTo, duration);
						var t = transition(element, propsTo, duration);
			...
			for (var selector in fadeProps) {
				if (fadeProps.hasOwnProperty(selector)) {
					...
					Static.each(document.querySelectorAll(selector), function (element) {
						...
						console.log('start transition', element, fadeProps, propsFrom, propsTo, duration);
...
if ( typeof module == "object" ) {
	module.exports = window.AjaxNav = AjaxNav;
}
